...
class plot_hs(object):
    ...
    def __init__(self,):
        ...
        # p = Pool(4)
        for it in range(268, times_len):
            self._plot_hs(it)
        #    p.apply_async(self._plot_hs(it), args=(it,))
        # results = p.map(self._plot_hs, range(0, times_len))
        # p.close()
        # p.join()
    ...
    def _plot_hs(self, it):
        ...
        im = ax.tripcolor(triang, self.ds.hs.isel(time=it), cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
        ...
        fullpath = os.path.join(self.fig_path, "hs", "hs_" + time_str + ".png")
        print(fullpath)
