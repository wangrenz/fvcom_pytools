...
class plot_uv(object):
    ...
    def _plot_uv(self, it):
        ...
        delta = 50
        ax.quiver(self.ds.lonc.values[::50], self.ds.latc.values[::50], self.ds.u.isel(time=it,siglay=0).values[::50], self.ds.v.isel(time=it,siglay=0).values[::50], width=0.002,headwidth=1,  transform=ccrs.PlateCarree())

        #im = ax.tripcolor(triang, self.ds.hs.isel(time=it,siglay=0), cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
        ...
        #clb.ax.set_title('hs (m)', horizontalalignment='left', position=(2.2, 0.5))
        ...
        fullpath = os.path.join(self.fig_path, "uv", "uv_" + time_str + ".png")
        print(fullpath)
